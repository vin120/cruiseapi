...
$("#button").on("click",function(){
		...
		$.ajax({
	        ...
	        data: {wifi_id:wifi_id,_csrf:csrfToken,mcode:mcode},
	        type: 'post',
	        dataType: 'json',
	        success :successFunc,
	        error: errorFunc,
		...
		function successFunc(response) {
			if(response.data){
            	//显示上网连接页面
            	location.href ="<?php echo Url::toRoute(['wifi/paymentsuccess']);?>?mcode=<?php echo $mcode?>";
            }else if(response.error){
                if(response.error.errorCode == 1){
                    //钱不够
                	//显示支付失败界面
                	location.href="<?php echo Url::toRoute(['wifi/paymentfail']);?>?mcode=<?php echo $mcode?>";
                }else if(response.error.errorCode == 2){
                    //出现错误
                    //显示支付出错界面
                	location.href="<?php echo Url::toRoute(['wifi/paymenterror']);?>?mcode=<?php echo $mcode?>";
                }
            }else {
            	//显示出错页面
            	location.href="<?php echo Url::toRoute(['wifi/paymenterror']);?>?mcode=<?php echo $mcode?>";
